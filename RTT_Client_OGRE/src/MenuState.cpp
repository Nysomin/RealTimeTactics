...
MenuState::MenuState()
{
    ...
    if(OgreFramework::getSingletonPtr()->m_callbackHandler == NULL)
    {
    	...
    	OgreFramework::getSingletonPtr()->m_callbackHandler = new TT::CallbackHandler();
    }
    else
    {
...
void MenuState::LeaderChangedEvent()
{
	...
	struct CallbackChange change = OgreFramework::getSingletonPtr()->m_callbackHandler->PopCallbackChange();
	...
	CEGUI::RadioButton newLeader = (CEGUI::RadioButton*)pMainWnd->getChild("IsLeader" + CEGUI::PropertyHelper::intToString((int)change.m_leaderID));

	oldLeader->setSelect(false);
	newLeader->setSelect(true);
